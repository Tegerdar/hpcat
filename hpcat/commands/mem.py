...
def poll_node(node: str, extended: bool) -> Tuple[str, Dict[str, Any]]:
    ...
    # We use /proc/meminfo instead of `free` to avoid parsing issues across different distros
    cmd = [
        'ssh',
        '-o', 'BatchMode=yes',
...
def execute(args: Any) -> int:
    ...
    target_nodes = args.nodes if getattr(args, 'nodes', None) else get_mem_nodes()
    if not target_nodes:
